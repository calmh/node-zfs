...
function ZFS(info, header) {
    "use strict";

    header = header || "NAME USED AVAIL REFER MOUNTPOINT";
    ...
    for (var i = 0, l = header.length; i < l; i += 1) {
        switch (header[i]) {
        case "avail":
        case "available":
        ...
        case "written":
            if (info[i] !== "-") {
                obj[header[i]] = parseInt(info[i], 10);
            }
            break;
        case "creation":
            var secondsSinceEpoch = parseInt(info[i], 10);
            if (!isNaN(secondsSinceEpoch)) {
                obj[header[i]] = new Date(secondsSinceEpoch * 1000);
            }
            break;
        case "clones":
            obj[header[i]] = info[i].split(",");
            break;
        case "defer_destroy":
        case "mounted":
            obj[header[i]] = false;
        ...
        // handle case where header can include user
        if (header[i].match(/^userused@/)) {
            if (info[i] !== "-") {
                obj[header[i]] = parseInt(info[i], 10);
            } else {
                obj[header[i]] = null;
        ...
        // handle case where header can include group
        if (header[i].match(/^groupused@/)) {
            if (info[i] !== "-") {
                obj[header[i]] = parseInt(info[i], 10);
            } else {
                obj[header[i]] = null;
        ...
        // handle case where header can include snapshot
        if (header[i].match(/^written@/)) {
            if (info[i] !== "-") {
                obj[header[i]] = parseInt(info[i], 10);
            } else {
                obj[header[i]] = null;
...
function list(opts, cb) {
    ...
    zfs(params, function (err, stdout) {
        if (cb && typeof cb === 'function') {
            ...
            var list = lines.map(function (x) { return new ZFS(x, header); });
            cb(err, list);
        }
    });
}
